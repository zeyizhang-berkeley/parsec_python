...
# Work around CUB reduction compilation issues on newer GPUs by using
# CuPy's fallback implementations for reductions and routines.
...
def load_atomic_data_from_file():
    ...
    # Handle .mat files
        ...
        if 'AtomsInMolecule' in mat_data:
            ...
            n_atom = [np.asarray(atom['coord']).reshape(-1, 3).shape[0] for atom in Atoms]
        else:
...
while err > tol and its <= maxits:
    ...
    rho = cp.asnumpy((W[:, :nev] ** 2) @ (2 * occup))
...
with open("./wfn.dat", "wb") as wfnid:
    ...
    wfnid.write(np.array(_to_numpy_array(pot), dtype=np.float64).tobytes())

    rho_length = len(rho)
    wfnid.write(np.array(rho_length, dtype=np.uint32).tobytes())
    wfnid.write(np.array(_to_numpy_array(rho), dtype=np.float64).tobytes())

    w_length = len(W)
    wfnid.write(np.array(w_length, dtype=np.uint32).tobytes())
    wfnid.write(np.array(nev, dtype=np.uint32).tobytes())
    for i in range(nev):
        wfnid.write(np.array(_to_numpy_array(W[:, i]), dtype=np.float64).tobytes())
