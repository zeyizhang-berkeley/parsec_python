...
`src/parsec_python` is the one maintained solver package. The old monolithic
stack is archived on the `old-architecture` branch. Each physical operation
has a public API, and `prepare_single_point` stops after static setup so those
terms can be inspected or reused without running an SCF calculation. Public
workflow calls use the optimized layer by default; explicit `*_reference`
aliases retain the readable SciPy translation.
...
├── V_xc/pbe.py                unpolarized discrete-variational PBE GGA
...
├── acceleration/             native/CuPy backends, symmetry, and GPU solvers
├── main.py                   canonical accelerated command launcher
├── reference_main.py         explicit readable SciPy launcher
...
The canonical `parsec_python/main.py` launcher selects accelerated execution
by default. The earlier implementation is archived on GitHub's
`old-architecture` branch rather than duplicated under `src`. With `parsec.in`
and `H_POTRE.DAT` in the same directory:

```powershell
cd src\parsec_python
...
```python
from parsec_python import prepare_single_point
...
```python
from parsec_python import ParsecRadialSpline, read_parsec_pseudopotential
...
```python
from parsec_python import HartreeSettings, solve_hartree
...
```python
from parsec_python import ca_lda, pbe
...
pbe_xc = pbe(valence_density, grid, core_density=core_density)
print(pbe_xc.potential)
print(pbe_xc.total_energy)
```
...
- spin-polarized GGA, meta-GGA, and hybrid functionals
...
or an XC label inconsistent with the selected CA/PZ-LDA or PBE family are
rejected rather than silently misused.
...
```powershell
$env:PYTHONPATH = "src"
python -m unittest discover -s src\parsec_python\tests -v
```
...
8. pointwise selected-XC potential and total `E_xc`
