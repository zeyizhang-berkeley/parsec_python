...
def manual_input_species(elem):
    ...
    # ADD hpot METHOD CHOICE HERE
    print('\n ********************** ')
    print(' HARTREE POTENTIAL INITIALIZATION METHOD')
    print(' ********************* ')
    print('------------------------')

    method_choice_hpot = 0
    while method_choice_hpot not in [1, 2]:
        try:
            print('Choose hartree potential initialization method:')
            print('1. Superposition of atomic hartree potentials (traditional) but still use the ML grids')
            print('2. Read hartree potential (.npy file)')
            method_choice_hpot = int(input('Enter your choice (1 or 2): '))
        except ValueError:
            print('Please enter 1 or 2')

    if method_choice_hpot == 1:
        hpot_method = 'atomic'
        rd_file_path = None
    else:
        hpot_method = 'read'  # Get ML density file path
        rd_file_path = input('Enter path to read hpot .npy file: ')

    # for Drew: Add manual input for doing CDFT? return a bool variable?(at least for a simple test, can add other options)

    return Atoms, n_atom, Z_charge, density_method, ml_file_path, hpot_method, rd_file_path
...
def get_atoms(elem):
    ...
    # Case 1: Manual input
    if in_data == 1:
        Atoms, n_atom, Z_charge, density_method, ml_file_path, hpot_method, rd_file_path = manual_input_species(elem)
        if n_atom is None:
            return None
    # Case 2: File input
...
Atoms, n_atom, Z_charge, density_method, ml_file_path, hpot_method, rd_file_path = get_atoms(elem)
OUT_PATH = f'./rsdft_parameter_{density_method}.out'
print('Atoms', Atoms)
print('n_atom', n_atom)
...
write_rsdft_parameter_output(OUT_PATH, nev, Atoms, n_atom, Domain, h, poldeg, fd_order)
...
if density_method == 'atomic':
    ...
    #rho0, hpot0, Ppot = pseudoDiag(Domain, Atoms, elem, N_elements, A, CG_prec, PRE)

    rho0, hpot0, Ppot = pseudoDiag(Domain, Atoms, elem, N_elements)

else: # ML method
    if hpot_method == 'atomic':
        print(f'Using ML-predicted density from: {ml_file_path}')
        print('Using superposition of atomic hartree potentials')

        from pseudoDiag_ML4Den_poisson import pseudoDiag_ML4Den_poisson
        ...
        rho0, hpot0, Ppot = pseudoDiag_ML4Den_poisson(Domain, Atoms_raw, elem, N_elements, ml_file_path, A, CG_prec, PRE)

    else:
        print(f'Using ML-predicted density from: {ml_file_path}')
        print(f'Using read hpot from: {rd_file_path}')

        from pseudoDiag_ML4DenHpot import pseudoDiag_ML4DenHpot
        ...
        # Call ML-based initialization
        print('Atoms_raw', Atoms_raw)
        rho0, hpot0, Ppot = pseudoDiag_ML4DenHpot( Domain, Atoms_raw, elem, N_elements, ml_file_path, rd_file_path, A, CG_prec, PRE )
...
rho0_bohr3 = rho0 / (h**3)
np.save(f"rho0_{density_method}.npy", rho0_bohr3)
print(f"Saved initial density as rho0_{density_method}.npy")
...
start_time = time.time()
XCpot, exc = exc_nspn(Domain, rhoxc, OUT_PATH)
...
while err > tol and its <= maxits:
    ...
    XCpot, exc = exc_nspn(Domain, rho, OUT_PATH)
...
np.save("rho_converged_3d.npy", rho_3d)
print("Saved final density as rho_converged_3d.npy with shape", rho_3d.shape)

hpot = Hpot + hpot0
hpot_3d = hpot.reshape(nx, ny, nz)
np.save("hpot_converged_3d.npy", hpot_3d)
print("Saved final hartree potential as hpot_converged_3d.npy with shape", hpot_3d.shape)
