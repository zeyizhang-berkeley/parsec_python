...
`src/fortran_single_point` is now independent of the older monolithic Python
...
The `fortran_single_point/main.py` launcher is contained inside this package
and leaves the older `src/legacy_python/main.py` workflow separate. With
`parsec.in` and `H_POTRE.DAT` in the same directory:

```powershell
cd src\fortran_single_point
...
from fortran_single_point import (
    Atom,
    EigensolverSettings,
    GridSettings,
    MixingSettings,
...
```python
from fortran_single_point import prepare_single_point
...
```python
from fortran_single_point import ParsecRadialSpline, read_parsec_pseudopotential
...
```python
from fortran_single_point import HartreeSettings, solve_hartree
...
```python
from fortran_single_point import ca_lda
...
```powershell
$env:PYTHONPATH = "src"
python -m unittest discover -s src\fortran_single_point\tests -v
```
